...
4. **结构提纲**（若开启）：每段「主线 / 散点」用无序列表；其下**节点用有序列表**（`1.` / `2.`…），不要写成 `- 1.` 混排。用于观察跳跃与缺主线，**不是** Pass A / 教练结论。

细课型取值：`004`～`010` 或 `other`。`other` 表示模型不敢判，请维护者校准后再解读分布。细课型**不**写入主路径 `lesson_type` 门禁。

## 日课对齐备忘（本地，不入库）

维护者可在 `output/<课例目录>/` 另存**校区对齐备忘**（对事不对人；公开仓库不提交）。备忘宜含：课型时长分布要点、结构可靠性说明、可执行课程建议、给校区负责人的对齐短句。生成前建议先对照纠错稿抽查结构节点是否靠谱。
